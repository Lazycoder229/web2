 ...
 *
 * ------------------------------------------------------------------
 * Middleware resolution order (updated)
 * ------------------------------------------------------------------
 * Three sources can now contribute middleware to a single route, and
 * they're merged in this order before being handed to Router:
 *
 *   1. Class-level  — #[UseMiddleware(...)] and/or the `middleware`
 *                      param on a class-level #[Route(...)]
 *   2. Method-level  — #[UseMiddleware(...)] on the method
 *   3. Route-level   — the `middleware` param on the Get/Post/Put/
 *                       Patch/Delete/Route attribute itself
 *
 * The combined list is deduped with array_unique() so declaring the
 * same middleware twice (e.g. once at class level, once inline) is
 * harmless rather than running it twice.
 */
...
class AttributeRouteLoader
{
    ...
    private function register_class(ReflectionClass $ref)
    {
        ...
        $class_middleware = $this->class_middleware($ref);
        ...
        foreach ($this->own_public_methods($ref) as $method) {
            ...
            foreach ($definitions as [$path, $http_methods, $route_middleware]) {
                ...
                // class -> method (#[UseMiddleware]) -> inline on the
                // route attribute itself, deduped into one final list.
                $all_middleware = array_values(array_unique(
                    array_merge($class_middleware, $method_middleware, $route_middleware)
                ));
    ...
    /**
     * Every route definition on a method, as [path, http_methods, middleware]
     * tuples. A method can carry Get/Post/Put/Patch/Delete (each at most
     * once — none of them are IS_REPEATABLE) and/or a #[Route(...)] for
     * the multi-verb case, so more than one tuple is possible.
     *
     * @param ReflectionMethod $method
     * @return array List of [path, http_methods[], middleware[]]
     */
    ...
    /**
     * Class-level middleware, merged from #[UseMiddleware(...)] AND the
     * `middleware` param on a class-level #[Route(...)] — both are valid
     * ways to declare "every route in this controller gets these".
     *
     * @param ReflectionClass $ref
     * @return string[]
     */
    private function class_middleware(ReflectionClass $ref)
    {
        $names = $this->middleware_names($ref->getAttributes(UseMiddleware::class));

        $route_attrs = $ref->getAttributes(Route::class);
        if ($route_attrs) {
            $names = array_merge($names, $route_attrs[0]->newInstance()->middleware);
        }
